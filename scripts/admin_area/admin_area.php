...
{
    private $queryPost = 'INSERT INTO admin_area(id,id_area,id_staff,id_position,id_journeys) VALUES(:num,:idarea,:idstaf,:idposition,:idjourneys)';
    private $queryGetAll = 'SELECT * FROM admin_area';
    private $queryUpdate = 'UPDATE admin_area SET id = :id_area, id_staff = :id_position, id_journeys = :num, idarea = :idstaf, idposition = :idjourneys  WHERE n_bill = :billete';
    ...
    public function postAcademicArea()
    {
        try {
            ...
            $res->bindValue("idarea", $this->id);
            $res->bindValue("idposition", $this->id_area);
            $res->bindValue("num", $this->id_staff);
            $res->bindValue("idstaf", $this->id_position);
